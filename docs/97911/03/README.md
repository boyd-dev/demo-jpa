...
앞서 살펴본 것은 *-toOne 관계이므로 컬렉션이 아니었는데, 이제 반대로 양방향 `@OneToMany`에서 컬렉션을 포함하는 프로젝션을 만들 때 주의할 점을 알아본다.  
  ...
  마찬가지로 프로젝션을 nested 형태로 인터페이스를 만드는 경우 비효율적인데, 쿼리 자동 빌더의 경우 지연 관계때문에 N+1 문제가 발생한다. 예를 들어 author를 가져오는 select와 함께 연관관계의 book이 3건이라면 추가적으로 3개의 select가 발생하는 것이다. 
  
  JPQL로 직접 조회하면 N+1 문제는 없지만 결과 JSON의 구조가 달라진다. JOIN FETCH를 사용하는 경우도 프로젝션 외에 불필요한 속성들을 1차 캐시로 가져오기 때문에 바람직하지 않다. 
