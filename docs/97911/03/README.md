...
속성 지연 로딩을 사용하기 위해서는 bytecode enhancement를 활성화시켜야 한다. 그런데 이 경우에 스프링 부트의 JSON 처리 라이브러리인 Jackson에서 문제가 발생할 소지가 있다.  
  ...
  bytecode enhancement를 쓰는 것이 번거롭다면 서브 엔티티(상속)를 이용하는 방법이 있다. 즉 지연 로드 속성만 가진 엔티티(서브 엔티티) 그렇지 않은 속성을 가진 엔티티(부모 엔티티)로 분리하는 것인데 어떻게 보면 이것이 가장 간단한 방법일 수 있다.
...
- *-toOne 연관관계 프로젝션  
author와 book은 양방향 `@OneToMany`(또는 `@ManyToOne`) 관계에 있다. 이때 book의 title과 그 book의 name, genre를 가져오는 프로젝션을 생각해보자. 즉 *-toOne 연관관계의 데이터를 가져오는 프로젝션이다.  

  먼저 아래와 같이 nested 형태로 인터페이스를 정의할 수 있다. 
  ...
  ```
  이 방법이 정석처럼 보이지만 예상과는 달리 author의 모든 속성을 조회하기 때문에 비효율적인 측면이 많다. 그렇다면 nested 속성을 flat하게 펼쳐서 정의하면 어떻게 될까?
  ...
  이 경우는 직접 JPQL을 작성해야 하고 실행되는 쿼리를 확인해보면 인터페이스에 정의된 속성만을 가져온다는 것을 알 수 있다. 하지만 결과 JSON의 구조가 처음과는 달리 한 레벨로 펼쳐져 있기 때문에 데이터 구조적으로 바람직하지 않다(물론 이것이 원하는 결과일 수도 있다).

  세 번째는 좀 더 복잡한 방법으로 "가상" 속성을 이용하는 것이다. 
  ...
  ```
  위의 코드처럼 author를 가져올 때 `@Value` 어노테이션과 spEL을 사용하여 미리 등록한 매퍼 빈의 메소드를 호출한다. 이렇게 하면 예제처럼 `AuthorDto`의 속성만을 select하여 최종 프로젝션인 `VirtualBookDto`으로 리턴한다. 변환된 JSON 구조도 유지된다. 하지만 이 방법은 다른 방법들에 비해 처리 속도가 느리다.  

  결론적으로 위의 세 가지 경우를 비교했을 때 각 방법들은 트레이드오프가 있기 때문에 상황에 따라 선택할 필요가 있겠다.

- 연관관계 컬렉션을 가져오는 프로젝션  
앞서 살펴본 것은 *-toOne 관계이므로 컬렉션이 아니었는데, 이제 반대로 양방향 `@OneToMany`에서 컬렉션을 포함하는 프로젝션을 만들 때 주의할 점을 알아본다.  

  마찬가지로 프로젝션을 nested 형태의 인터페이스를 만드는 경우 비효율적인데, 쿼리 자동 빌더의 경우 지연 관계때문에 N+1 문제가 발생한다. 예를 들어 author를 가져오는 select와 함께 연관관계의 book이 3건이라면 추가적으로 3개의 select가 발생하는 것이다. 
  
  JPQL로 직접 조회하면 N+1 문제는 없지만 결과 JSON의 구조가 달라진다. JOIN FETCH를 사용하는 경우도 프로젝션 외에 불필요한 속성들을 1차 캐시로 가져오기 때문에 바람직하지 않다(책에서는 캐시에서 관리할 필요가 없으면 “persistent context bypass”라는 표현을 사용한다). 

  nested 인터페이스가 아닌 펼친 구조라고 해도 결과 JSON의 구조가 flat하게 되므로 나중에 별도의 처리가 필요할 수도 있다.
