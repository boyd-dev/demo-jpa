...
author와 book은 양방향 `@OneToMany`(또는 `@ManyToOne`) 관계에 있다. 이때 book의 title과 그 book의 name, genre를 가져오는 프로젝션을 생각해보자. 즉 *-toOne 연관관계의 데이터를 가져오는 프로젝션이다.  
  ...
  ```
  이 방법이 정석처럼 보이지만 예상과는 달리 author의 모든 속성을 조회하기 때문에 비효율적인 측면이 많다. 그렇다면 nested 속성을 flat하게 정의하면 어떻게 될까?
  ...
  ```
  위의 코드처럼 author를 가져올 때 `@Value` 어노테이션과 spEL을 사용하여 미리 등록한 매퍼 빈의 메소드를 호출한다. 이렇게 하면 예제처럼 `AuthorDto`의 속성만을 select하여 최종 프로젝션인 `VirtualBookDto`으로 리턴한다. 변환된 JSON 구조도 유지된다.  
...
앞서 살펴본 것은 *-toOne 관계이므로 컬렉션이 아니었는데, 이제 반대로 양방향 `@OneToMany`에서 컬렉션을 포함하는 프로젝션을 만들 때 주의할 점을 알아본다.  
  ...
  마찬가지로 프로젝션을 nested 형태로 인터페이스를 만드는 경우 비효율적인데 지연 관계때문에 N+1 문제가 발생한다. 예를 들어 author를 가져오는 select와 함께 연관관계의 book이 3건이라면 추가적으로 3개의 select가 발생하는 것이다. 
