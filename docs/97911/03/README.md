...
author와 book은 양방향 `@OneToMany`(또는 `@ManyToOne`) 관계에 있다. 이때 book의 title과 그 book의 name, genre를 가져오는 프로젝션을 생각해보자. 즉 *-toOne 연관관계의 데이터를 가져오는 프로젝션이다.  
  ...
  먼저 살펴볼 것은 아래와 같이 nested 형태의 인터페이스를 정의할 수 있다. 
  ...
  이 경우는 직접 JPQL을 작성해야 하고 실행되는 쿼리를 확인해보면 인터페이스에 정의된 속성만을 가져온다는 것을 알 수 있다. 하지만 결과 JSON의 구조가 처음과는 달리 한 레벨로 펼쳐져 있다(물론 이것이 원하는 결과일 수도 있다).
