...
위의 설명에서 "owning side"는 흔히 말하는 연관 관계의 주인이라고 표현을 하지만 사실은 <b>FK를 소유</b>하는 엔티티를 의미합니다. 반면 "inverse side"는 `mappedBy`로 참조하는 쪽을 말합니다. 양방향 `@OneToMany`에서 Album이 owning side이고 Singer가 inverse side가 되겠습니다.
