...
`@JoinColumn`으로 Album 테이블에 생기는 FK컬럼 이름을 지정할 수 있습니다. Album 엔티티는 Singer를 참조하고 있으므로 Album을 통해 Singer를 알 수 있습니다. 반대로 Singer로부터 Album을 알 수 있을까요? DB의 테이블이라면 당연히 알 수 있지만 Singer에는 Album에 대한 어떠한 참조도 추가되지 않았으므로 알 수 없습니다. 이러한 관계를 "단방향(unidirectional)"이라고 합니다. 
...
private Set<Album> albums = new HashSet<>();
```
일반적으로 단방향 @OneToMany는 부가적인 쿼리를 수행하기 때문에 권장하지 않습니다. 단방향 @OneToMany는 @ManyToOne의 관계로(어차피 관계 자체는 달라지지 않으므로) 변경하거나 parent의 PK로 child를 가져올 때는 직접 JPQL로 해결하는 방법을 생각해 볼 수 있습니다. JPQL이란 SQL과 유사하지만 객체들을 대상으로 하는 쿼리문이라고 생각하면 됩니다.
...
반대로 한꺼번에 가져올 때는 `FetchType.EAGER`를 사용합니다. 보통은 `FetchType.LAZY`를 기본으로 하고 필요한 경우 JPQL에서 조인을 통해 연관 데이터를 가져오는 것을 권장하고 있습니다.
...
@OneToOne에도 단방향과 양방향이 있습니다. DB 관점에서는 detail에 FK가 생기는 것이 일반적입니다. 그런데 하이버네이트에서는 Phone에 `@OneToOne`을 추가하면 Phone에 FK가 생깁니다. 하이버네이트 가이드에는 이러한 이상한 상황에 대해 다음과 같이 설명합니다.
...
네 가지 연관 관계에 대해 간단하게 알아보았습니다. 연관 관계의 "owning side"를 기준으로 데이터의 CRUD가 이루어지므로 목적에 맞게 설정하는 것이 중요합니다. 
