...
리턴되는 Singer 객체는 이제 PK에 해당하는 `id` 값이 들어 있고 이제 `transient`에서 비로소 `managed` 상태가 됩니다. `managed` 상태의 엔티티 객체들은 소위 말하는 "persistence context"에 캐시(cache)됩니다. 하이버네이트의 가이드에 이에 대한 [설명](https://docs.jboss.org/hibernate/orm/5.3/userguide/html_single/Hibernate_User_Guide.html#pc)이 잘 나와 있습니다.
