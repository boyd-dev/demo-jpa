...
특히 `Set`과 자바 컬렉션을 사용하는 경우는 목적에 맞는 `hashCode`와 `equals` 메소드를 오버라이드할 필요가 있습니다. 그런데 엔티티의 `id` 컬럼을 `hashCode`와 `equals`에 이용할 때 주의해야 합니다. 왜냐하면 만약 `Set`에 데이터를 추가하는 경우 신규 등록일 때(transient)는 `id`는 null이 되므로 두 객체를 비교할 때 `equals`는 false가 됩니다. 그렇게 되면 엔티티가 동일하다고 해도 `Set`에는 같은 엔티티가 중복으로 들어갈 수 있습니다.  
