...
- `JpaRepository<T, ID>`를 상속하는 인테페이스를 작성하여 해당 엔티티(T)의 CRUD를 간소화할 수 있다.
- 기본 제공되지 않는 CRUD 메소드들은 메소드 이름에 의한 쿼리 빌더 메커니즘을 이용하여 추가할 수 있다.
- 메소드 이름으로 자동 생성될 수 없는 쿼리는 `@Query` 어노테이션을 사용하여 직접 SQL을 작성할 수 있다(JPQL, 네이티브).
- 또는 네임드 쿼리를 작성하여 쿼리를 중앙 집중적인 관리와 재사용 가능하게 만들 수 있다. 네임드 쿼리는 정적으로 생성되기 때문에 `@Query`보다 성능상 나을 수 있다.
- 수정을 위한 목적이 아니라면 엔티티 단위로 패치하는 것은 바람직하지 않다. 원하는 프로젝션으로 원하는 결과셋을 가져오는 것이 리소스 절약과 성는 측면에서 좋다.
- 프로젝션의 결과셋은 여러가지 방식으로 정의할 수 있다.
